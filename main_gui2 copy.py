...
DEFAULT_START = (0, 0)
DEFAULT_GOAL = (9, 9)
DEFAULT_OBSTACLES = []  # Empty environment by default
...
def train_robot():
    ...
    # Update reward plot
    ax_reward.clear()
    ax_reward.plot(reward_per_episode, label='Reward')
    ax_reward.set_title("Rewards over Episodes")
...
def update_visualization():
    ...
    legend_elements = [
        ...
        Line2D([0], [0], color=OPTIMAL_PATH_COLOR, lw=3, label='Optimal Path'),
        plt.Circle((0, 0), 0.1, color=ROBOT_COLOR, label='Robot'),
        plt.Rectangle((0, 0), 1, 1, color=OBSTACLE_COLOR, hatch='//', label='Obstacle'),
        plt.Circle((0, 0), 0.1, color='green', label='Start'),
        plt.Circle((0, 0), 0.1, color='red', label='Goal')
    ]
...
def create_obstacle_checkboxes():
    ...
    for i, (x, y) in enumerate(DEFAULT_OBSTACLES):
        ...
        checkbox = CheckButtons(ax_check, [f"({x},{y})"], [False])  # Start with no obstacles
